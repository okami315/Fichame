...
class SigningController extends AbstractController
{
    ...
    #[Route('/', name: 'app_signing_index_almacen', methods: ['GET'])]
    public function indexAlmacen(SigningRepository $signingRepository): Response
    {
        $signings = $signingRepository->findAlmacenByUser($this->getUser()->getId());
        $bool = $signingRepository->findWithoutCheckoutNeitherEvent($this->getUser()->getId());
        
        return $this->render('signing/almacen.html.twig', [
            'signings' => $signings,
            'bool' => $bool,
        ]);
    }
    ...
    //     ]);
    // }
    #[Route('/{id}/editAlmacen', name: 'app_signing_edit_almacen', methods: ['GET', 'POST'])]
    public function editAlmacen(Request $request, Signing $signing, SigningRepository $signingRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('app_signing_index_almacen', [], Response::HTTP_SEE_OTHER);
        }
    ...
    #[Route('/entradaAlmacen', name: 'app_signing_entrada_almacen', methods: ['GET', 'POST'])]
    public function entradaAlmacen(Request $request, SigningRepository $signingRepository): Response
    {
        $signing = new Signing();
    ...
    #[Route('/{id}/salidaAlmacen', name: 'app_signing_salida_almacen', methods: ['GET', 'POST'])]
    public function salidaAlmacen(Request $request, Signing $signing, SigningRepository $signingRepository): Response
    {
